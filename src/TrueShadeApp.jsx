...
const SKIN_TONE_PALETTES = {
  0: {
    ...
    targetColors: ["#5D3A28", "#8B0000", "#4B2E2A", "#3E2723", "#581845", "#6D271A"],
  },
  1: {
    title: "Warm Medium",
    desc: "Undertone kuning langsat/sawo matang dengan nuansa hangat, honey, dan tan.",
    targetColors: ["#D29C7B", "#B35A5A", "#C68642", "#CD853F", "#A56B57", "#D2691E"],
  },
  2: {
    title: "Fair Ivory",
    desc: "Warna kulit terang dengan undertone netral atau pinkish porcelain.",
    targetColors: ["#F5E0D6", "#FFE4C4", "#FFDEAD", "#FAEBD7", "#F0E68C", "#FFC0CB"],
  },
};
...
export default function App() {
  ...
  // --- PERBAIKAN 1: LOAD MODEL ---
  useEffect(() => {
    const loadModel = async () => {
      try {
        setModelLoading(true);
        // PERUBAHAN: Gunakan loadGraphModel, bukan loadLayersModel
        // Graph Model tidak butuh patching JSON manual yang rumit
        const loadedModel = await tf.loadGraphModel("/model/model.json");

        // Warmup model dengan tensor kosong (sesuai input shape 300x300)
        const warmupResult = loadedModel.predict(tf.zeros([1, 300, 300, 3]));
        if (warmupResult.dispose) warmupResult.dispose();

        setModel(loadedModel);
        setModelLoading(false);
        console.log("Model loaded successfully via loadGraphModel");
      } catch (error) {
      ...
      if (product.product_colors && Array.isArray(product.product_colors)) {
        product.product_colors.forEach((prodColor) => {
          targetColors.forEach((skinHex) => {
            const distance = calculateColorDistance(prodColor.hex_value, skinHex);
            if (distance < 80) {
      ...
      if (model && imageRef.current) {
        try {
          const tensor = tf.browser
            ...
            // Resize sesuai input model Python (300x300)
            .resizeNearestNeighbor([300, 300])
            .toFloat()
            // Expand dims untuk batch: [1, 300, 300, 3]
            .expandDims();

          // Graph Model 'predict' outputnya bisa berupa Tensor atau array Tensor
          let predictionTensor = model.predict(tensor);

          // Jika output berupa array (kadang terjadi di graph model), ambil yg pertama
  ...
  // --- LOGIKA UI (Pagination, Scroll, Render) Tetap Sama ---
  ...
  const resultData =
    predictionResult !== null && SKIN_TONE_PALETTES[predictionResult]
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-stone-50 font-sans text-stone-800">
      <style>{`
        ...
        .animate-slide-next { animation: slideInRight 0.4s ease-out forwards; }
        .animate-slide-prev { animation: slideInLeft 0.4s ease-out forwards; }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex-shrink-0 flex items-center gap-2 cursor-pointer">
              ...
              <span className="font-serif text-xl font-bold tracking-tight">TrueShade</span>
            </div>
            <div className="hidden md:flex space-x-8 text-sm font-medium text-stone-600">
              <a href="#" className="hover:text-rose-500 transition">Beranda</a>
              <a href="#analyzer" className="hover:text-rose-500 transition">Analisis Wajah</a>
              <a href="#products" className="hover:text-rose-500 transition">Produk</a>
            </div>
            <div className="flex items-center gap-4">
              <button className="p-2 hover:bg-stone-100 rounded-full relative">
                <ShoppingBag size={20} />
                {recommendedProducts.length > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-rose-500 rounded-full"></span>}
              </button>
              <button className="md:hidden p-2 hover:bg-stone-100 rounded-full" onClick={() => setIsMenuOpen(!isMenuOpen)}>
      ...
      <section id="analyzer" className="py-20 bg-stone-900 text-white overflow-hidden relative">
        ...
        <div className="max-w-4xl mx-auto px-4 relative z-10">
          <div className="text-center mb-10">
            <h2 className="font-serif text-3xl md:text-4xl font-bold mb-4">Analisis AI</h2>
            <div className="flex flex-col items-center justify-center gap-3">
              {modelLoading && (
                <span className="text-yellow-400 flex items-center gap-1 text-sm bg-yellow-400/10 px-3 py-1 rounded-full">
                  <Loader2 size={14} className="animate-spin" /> Memuat Model AI...
                </span>
              )}
              ...
              {modelError && (
                <span className="text-red-400 flex items-center gap-1 text-sm bg-red-400/10 px-3 py-1 rounded-full">
                  <X size={14} /> Error: {modelError}
                </span>
              )}
            </div>
          </div>
              ...
              <div className="relative bg-black flex flex-col items-center justify-center p-6 overflow-hidden h-[400px] md:h-auto w-full order-1">
                {!previewImage && !isCameraOpen && (
                  <>
                    <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-stone-600 border-dashed flex items-center justify-center mb-6">
                      <Camera size={32} className="text-stone-500 md:w-10 md:h-10" />
                    </div>
                    <div className="flex flex-col gap-3 w-full max-w-xs z-10">
                      <button onClick={startCamera} disabled={modelLoading} className="w-full bg-rose-500 hover:bg-rose-600 text-white px-6 py-3 rounded-full font-medium transition flex items-center justify-center gap-2 shadow-lg disabled:opacity-50">
                        <Camera size={18} /> Ambil Foto
                      </button>
                      <button onClick={() => fileInputRef.current.click()} disabled={modelLoading} className="w-full bg-stone-700 hover:bg-stone-600 text-white px-6 py-3 rounded-full font-medium transition flex items-center justify-center gap-2 shadow-lg disabled:opacity-50">
                ...
                {isCameraOpen && (
                  <div className="absolute inset-0 flex flex-col bg-black">
                    <video ref={videoRef} autoPlay playsInline className="flex-1 w-full h-full object-cover transform scale-x-[-1]" />
                    <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-4 z-20">
                      <button onClick={stopCamera} className="p-4 bg-white/20 backdrop-blur-sm rounded-full text-white"><X size={24} /></button>
                      <button onClick={capturePhoto} className="w-16 h-16 bg-white rounded-full border-4 border-rose-500 flex items-center justify-center">
                ...
                {previewImage && (
                  <div className="relative w-full h-full flex items-center justify-center bg-black">
                    <img ref={imageRef} src={previewImage} alt="Preview" className={`w-full h-full object-contain ${isAnalyzing ? "opacity-50 blur-sm" : ""} transition-all`} crossOrigin="anonymous" />
                    {isAnalyzing && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
                        <Loader2 size={48} className="animate-spin text-rose-500 mb-2" />
                        <p className="text-rose-200 bg-black/50 px-3 py-1 rounded-full backdrop-blur-sm">Menganalisis...</p>
                      </div>
                    )}
                    {!isAnalyzing && (
                      <button onClick={() => { setPreviewImage(null); setAnalysisComplete(false); }} className="absolute bottom-4 right-4 bg-black/60 text-white p-2 rounded-full flex items-center gap-2 px-4 border border-white/20 hover:bg-black/80 backdrop-blur-sm z-20">
              ...
              <div className="p-6 md:p-8 flex flex-col justify-center bg-stone-800 w-full min-h-[300px] md:min-h-auto order-2">
                {!analysisComplete ? (
                  <div className="space-y-4 opacity-30 text-center">
                    <Sparkles size={48} className="mx-auto mb-4 text-stone-600" />
                    <p className="text-stone-500">Hasil analisis warna kulitmu akan muncul di sini.</p>
                  </div>
                ) : (
                  <div className="animate-fade-in">
                    <span className="text-rose-400 text-sm font-bold tracking-wider uppercase mb-2 block">Tone Terdeteksi</span>
                    <h3 className="text-3xl font-serif font-bold mb-1 text-white">{resultData.title}</h3>
                    <p className="text-stone-400 text-sm mb-6">{resultData.desc}</p>
                    <div className="pt-4 border-t border-stone-700">
                      <p className="text-xs text-stone-500 mb-2 uppercase tracking-wide">Palette Cocok:</p>
                      <div className="flex gap-3 mb-6 flex-wrap">
                        {resultData.targetColors.slice(0, 5).map((color, i) => (
                          <div key={i} className="w-8 h-8 rounded-full border border-white/20 shadow-sm" style={{ backgroundColor: color }}></div>
                        ))}
                      </div>
                      <button onClick={() => document.getElementById("products").scrollIntoView({ behavior: "smooth" })} className="w-full bg-white text-stone-900 py-3 rounded-lg font-bold hover:bg-stone-200 transition flex items-center justify-center gap-2 shadow-lg">
      ...
      <section id="products" className="py-24 bg-stone-50" ref={productsRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-12 flex flex-col md:flex-row justify-between items-end gap-4">
            <div>
              <h2 className="font-serif text-3xl font-bold text-stone-900">Rekomendasi Untukmu</h2>
              <p className="text-stone-500 mt-2">
                {analysisComplete ? `Ditemukan ${recommendedProducts.length} produk cocok untuk tone "${resultData.title}"` : "Lakukan analisis wajah untuk melihat produk yang dipersonalisasi."}
              </p>
            </div>
          ...
          {!analysisComplete && (
            <div className="flex flex-col items-center justify-center py-20 opacity-50 bg-white rounded-xl border border-dashed border-stone-300">
              ...
              <p className="text-stone-400">Produk akan muncul di sini setelah analisis.</p>
            </div>
          )}

          {analysisComplete && recommendedProducts.length === 0 && (
            <div className="text-center py-10 text-stone-500 bg-white rounded-xl border border-stone-200">
              Maaf, tidak ditemukan produk yang sangat mirip di database saat ini.
            </div>
          )}

          {analysisComplete && recommendedProducts.length > 0 && (
            <>
              <div key={currentPage} className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 ${slideDirection === "next" ? "animate-slide-next" : "animate-slide-prev"}`}>
                {currentProducts.map((product) => (
                  <div key={product.id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col h-full border border-stone-100">
                    <div className="relative h-64 overflow-hidden bg-white p-4 flex items-center justify-center">
                      <img src={product.image_link} alt={product.name} className="max-h-full max-w-full object-contain group-hover:scale-105 transition-transform duration-500" onError={(e) => { e.target.onerror = null; e.target.src = PLACEHOLDER_IMAGE; }} />
                      <div className="absolute top-3 right-3 bg-white/90 backdrop-blur text-stone-900 text-xs font-bold py-1 px-2 rounded shadow-sm flex items-center gap-1">
                        <Star size={12} className="text-yellow-500 fill-yellow-500" /> {product.matchPercent}% Match
                      </div>
                    </div>
                    <div className="p-5 flex-1 flex flex-col">
                      <p className="text-xs text-stone-500 mb-1 uppercase tracking-wider">{product.brand || "Brand"}</p>
                      <h3 className="font-bold text-stone-900 text-lg leading-tight mb-2 line-clamp-2" title={product.name}>{product.name}</h3>
                      <div className="flex items-center gap-2 mb-4 mt-auto">
                        <div className="w-6 h-6 rounded-full border border-stone-200 shadow-sm" style={{ backgroundColor: product.bestHex }}></div>
                        <div className="flex flex-col">
                          <span className="text-[10px] text-stone-400 uppercase">Shade</span>
                          <span className="text-xs font-semibold text-stone-700 line-clamp-1">{product.bestShade}</span>
                        </div>
                      </div>
                      <div className="pt-4 border-t border-stone-100 flex items-center justify-between">
                        <span className="font-serif font-bold text-rose-500">{product.price_sign || "$"}{product.price || "0.00"}</span>
                        <a href={product.product_link} target="_blank" rel="noreferrer" className="w-8 h-8 rounded-full bg-stone-900 text-white flex items-center justify-center hover:bg-rose-500 transition">
                          <ShoppingBag size={14} />
                        </a>
                      </div>
                    </div>
              ...
              {totalPages > 1 && (
                <div className="flex justify-center items-center mt-12 w-full">
                  <div className="flex items-center gap-4">
                    <button onClick={prevPage} disabled={currentPage === 1} className={`p-3 rounded-full border transition flex items-center justify-center ${currentPage === 1 ? "bg-stone-100 text-stone-300 border-stone-200 cursor-not-allowed" : "bg-white text-stone-800 border-stone-300 hover:bg-stone-50 hover:border-stone-400 shadow-sm"}`}>
                      <ChevronLeft size={20} />
                    </button>
                    <span className="text-sm font-semibold text-stone-600 md:hidden whitespace-nowrap">Hal {currentPage} / {totalPages}</span>
                    <div className="hidden md:flex gap-2">
                      {getPageNumbers().map((pageNum) => (
                        <button key={pageNum} onClick={() => goToPage(pageNum)} className={`w-10 h-10 rounded-full text-sm font-bold transition flex-shrink-0 flex items-center justify-center ${currentPage === pageNum ? "bg-stone-900 text-white shadow-md" : "bg-white text-stone-600 border border-stone-200 hover:bg-stone-50"}`}>
                          {pageNum}
                        </button>
                      ))}
                    </div>
                    <button onClick={nextPage} disabled={currentPage === totalPages} className={`p-3 rounded-full border transition flex items-center justify-center ${currentPage === totalPages ? "bg-stone-100 text-stone-300 border-stone-200 cursor-not-allowed" : "bg-white text-stone-800 border-stone-300 hover:bg-stone-50 hover:border-stone-400 shadow-sm"}`}>
      ...
      <footer className="bg-stone-900 text-stone-400 py-12 border-t border-stone-800">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-xs">&copy; 2024 TrueShade AI Inc. Data provided by Makeup API.</p>
        </div>
      </footer>
    </div>
  );
